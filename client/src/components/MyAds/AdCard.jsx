...
export const AdStatusBadge = memo(({ status }) => {
...
const StatusButton = memo(({ status, isLoading, onClick }) => (
  <button
    onClick={onClick}
    disabled={isLoading}
...
const ActionButton = memo(({ onClick, className, title, icon }) => (
  <button
    onClick={onClick}
    className={`flex items-center justify-center w-10 h-10 rounded-full ${className} transition-colors`}
    title={title}
  >
    {icon}
  </button>
...
const AdCard = ({
  ...
  }, [linkTo, onView, ad._id]);

  const handleActionClick = useCallback((e, action, id) => {
    e.stopPropagation();
    action && action(id);
  }, []);
  ...
  return (
    ...
    >
      <div className="relative h-48 overflow-hidden">
        <img
          src={ad.images?.length ? ad.images[0] : defaultImage}
          alt={ad.title}
        ...
        <>
          ...
          >
            <div className="flex justify-between">
              <ActionButton
                onClick={(e) => handleActionClick(e, onView, ad._id)}
                className="bg-blue-50 text-blue-600 hover:bg-blue-100"
                title="Görüntüle"
                icon={<FaEye />}
              />
              <ActionButton
                onClick={(e) => handleActionClick(e, onEdit, ad._id)}
                className="bg-green-50 text-green-600 hover:bg-green-100"
                title="Düzenle"
                icon={<FaEdit />}
              />
              <ActionButton
                onClick={(e) => handleActionClick(e, onDelete, ad._id)}
