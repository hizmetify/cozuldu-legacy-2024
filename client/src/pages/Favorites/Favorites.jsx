...
const Favorites = () => {
  ...
  const metaTitle = 'Favori İlanlarım - Çözüldü';
  const metaDescription =
    'Favori ilanlarınızı görüntüleyin, yönetin ve kolayca erişin. Beğendiğiniz hizmetleri kaybetmeyin.';
  const metaKeywords =
    'favoriler, favori ilanlar, kaydedilmiş ilanlar, hizmet favorileri';
  const canonical = 'https://xn--zld-1la9esbc.com/dashboard/favorites';

  const ogImage =
    filteredIlanlar.length > 0 && filteredIlanlar[0]?.adId?.images?.length > 0
      ? filteredIlanlar[0].adId.images[0]
      : undefined;
    ...
    return (
      <>
        <MetaHelmet
          ...
          canonical={canonical}
        />
        <LoadingSpinner message="Favorileriniz Yükleniyor" />
      </>
    );
  }
