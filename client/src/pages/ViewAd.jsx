...
const ViewAd = () => {
  ...
  }, [dispatch, adId, isAuthenticated]);

  // Debug logs
  useEffect(() => {
    console.log('Current state:', selectedAd);
  }, [selectedAd]);
  ...
  if (loading || singleAdStatus === 'loading') {
    return <LoadingSpinner message="İlan detayları yükleniyor" />;
  }

  console.log(
    'Full selectedAd structure:',
    ...
    <>
      <div className="min-h-screen bg-gray-50 p-4 md:p-8 overflow-x-hidden">
        <MetaHelmet
          ...
          ogImage={ogImage}
          ogType="product"
          canonical={`${window.location.origin}/ad/${adId}`}
        />
        <Link
          to="/"
